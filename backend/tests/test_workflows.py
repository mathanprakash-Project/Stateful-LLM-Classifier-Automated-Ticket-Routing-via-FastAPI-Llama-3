...
async def test_restricted_operation_full_workflow(client: AsyncClient):
    ...
    user_res = await client.post("/auth/login", json={"email": "john@company.com", "password": "password123"})
    user_headers = {"Authorization": f"Bearer {user_res.json()['access_token']}"}
    
    # 2. Create ticket (use first available category)
    # 2. User creates ticket (use first available category)
