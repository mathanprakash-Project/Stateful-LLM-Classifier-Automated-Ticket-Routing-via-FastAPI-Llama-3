...
async def send_chat_message(
    ...
    result = await chat_service.process_user_message(
        session_id, current_user, msg_in.content, preferred_model=msg_in.preferred_model
    )
    draft_resp = None
    if result.get("draft"):
        draft = result["draft"]
...
async def approve_draft(
    ...
    chat_service = ChatService(db)
    return await chat_service.approve_draft(session_id, draft_id, current_user, custom_draft_data=custom_data)
