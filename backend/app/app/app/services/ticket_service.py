...
class TicketService:
    ...
    async def reopen_ticket(self, ticket_id: str, user: User, session: AsyncSession, reason: str = "") -> Ticket:
        ticket = await self._get_ticket_for_update(ticket_id, user, session)
        user_roles = [r.name.lower() for r in user.roles]
        ...
        # Store detailed reopen reason history
        reasons_list = list(meta.get("reopen_reasons", []))
        if reason:
            reasons_list.append({
                "attempt": meta["reopen_count"],
                "reason": reason,
                "reopened_by": user.full_name,
                "reopened_at": datetime.now(timezone.utc).isoformat()
            })
            meta["latest_reopen_reason"] = reason
        meta["reopen_reasons"] = reasons_list
        
        # Elevate priority for reopened tickets
        ...
        history_reason = f"Ticket reopened (Reopen attempt {meta['reopen_count']} of 3)"
        if reason:
            history_reason += f" - Reason: {reason}"
        ...
        # Add comment with customer's detailed reason
        if reason:
            await self.ticket_repo.add_comment(
                ticket_id=ticket.id,
                user_id=user.id,
                content=f"🔄 **Ticket Reopened (Attempt {meta['reopen_count']} of 3)**\n\n**Customer Reopen Reason:** {reason}\n\n*⚡ Priority elevated to **HIGH** for urgent follow-up.*",
                is_internal=False
            )
